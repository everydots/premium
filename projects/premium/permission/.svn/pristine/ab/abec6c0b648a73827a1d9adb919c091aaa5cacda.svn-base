...
package com.everydots.permission.common.spring;
...
import com.everydots.permission.common.spring.DataControlXML.Condition;
import com.everydots.permission.common.util.Exceptions;
...
import com.everydots.permission.common.util.util.persistence.SearchFilter;
import com.everydots.permission.common.util.util.persistence.SearchFilter.Operator;
...
public class DataControlInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean check(HttpServletRequest request, HttpServletResponse response, Method method,
		...
		for (String sn : m) {
			...
			for (String name : d) {
				...
				try {
					Structure structure = SecurityUtils.getShiroUser().getHasStructures().get(sn);
					
					boolean handleResult = true;
					if (method.getName().startsWith(SHOW_METHOD_PREFIX)) {
						handleResult = (handleList(request, filterSet, method, dataControl, structure) == true) ? true : false;
					} else {
						handleResult = (handleOther(request, filterSet, method, dataControl, structure) == true) ? true : false;
					}
	...
	protected boolean handleList(HttpServletRequest request, Set<SearchFilter> filterSet, 
			Method method, DataControl dataControl, Structure module) {
	...
	protected boolean handleOther(HttpServletRequest request, Set<SearchFilter> filterSet, 
			Method method, DataControl dataControl, Structure structure) throws Exception {
		...
		if (method.getName().endsWith(MANY_METHOD_SUFFIX)) { // 多对象操作方法
			...
			if (id != null) {
				...
				if (StringUtils.isNotBlank(tmp)) {
					filterSet.add(new SearchFilter("id", Operator.EQ, tmp));
				}
			}
		}
		Object clazz = Class.forName(structure.getClassname()).newInstance();
		//Object clazz = null;
