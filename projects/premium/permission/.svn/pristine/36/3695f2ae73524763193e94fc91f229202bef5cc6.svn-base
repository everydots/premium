...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken)
		...
		if (useCaptcha) {
			...
			if (!PatchcaServlet.validate(SecurityUtils.getSubject().getSession().getId().toString(),
					parm.toLowerCase())) {// 忽略大小写。
		...
		for (Role role : roles) {
			...
			if (null != rolePermissions && rolePermissions.size() > 0) {
				for (RolePermission rolePermission : rolePermissions) {
					...
					if (null == permission) {
						if (log.isInfoEnabled()) {
							log.info(shiroUser.getLoginName() + "设置的permission有误");
						}
						continue;
					}
					...
					if (null == structure) {
						if (log.isInfoEnabled()) {
							log.info(shiroUser.getLoginName() + "设置的structure有误");
						}
						continue;
					}
					...
					List<RolePermissionDataControl> rolePermissionDataControls = rolePermissionDataControlService
							.findByRolePermissionId(rolePermission.getRolepermissionid());

					for (RolePermissionDataControl rpdc : rolePermissionDataControls) {
						DataControl dataControl = dataControlService.findByDatacontrolid(rpdc.getDatacontrolid());
	...
	public void setRolePermissionDataControlService(RolePermissionDataControlService rolePermissionDataControlService) {
		this.rolePermissionDataControlService = rolePermissionDataControlService;
	}
